...
from pylab import scatter, show, figure, title, xlabel, ylabel, subplot,\
                  subplots_adjust, ylim, xlim, savefig
...
if __name__ == "__main__":
    ...
    figure()
    scatter(dist[w], ppm[w], s=80,marker='+')
    ylabel(r'Proper motion [arcsec/yr]')
    ...
    figure()
    scatter(dist[w], mass[w], s=80,marker='+')
    ylabel(r'Estimated mass [$M_\odot$]')
    xlabel(r'Distance [parsecs]')
    ylim(ymin=0)
    xlim(xmin=0)
...
#   ylim(ymin=0)
    ...
    figure()
    scatter(dist[w], 2*rE[w]/1000*ppm[w], s=80,marker='+')
    ylabel(r'$2\theta_E \times$ proper motion [arcsec$^2$/yr]')
    xlabel(r'Distance [parsecs]')
    ylim(ymin=0)
    xlim(xmin=0)
    ...
    figure()
    scatter(mass[w], 2*rE[w]/1000*ppm[w], s=80,marker='+')
    ylabel(r'$2\theta_E \times$ proper motion [arcsec$^2$/yr]')
    xlabel(r'Estimated mass [$M_\odot$]')
    ylim(ymin=0)
    xlim(xmin=0)
    ...
    print len(rE[w])
    print sum(2*rE[w]/1000*10*ppm[w])
